    ...
    <div className="relative">
      ...
      >
        <div className={`h-full px-3 py-4 rounded-2xl shadow-xl overflow-y-auto ${dark ? 'bg-[#171717]' : 'bg-[#f5f5f5]'} `}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 px-20 py-5">
            <div className="flex flex-col items-center">
              <img
                src={ind}
                onClick={() => handleCountryClick('in')}
                alt="India"
                className={`flex items-center justify-center ${selectedCountry === 'in' ? (dark ? 'outline outline-4 outline-[#7FFF9B] rounded-sm' : 'outline outline-4 outline-[#235347] rounded-sm') : ''} w-10 cursor-pointer`}
              />
              <span className={`text-xs pt-2 ${selectedCountry === 'in' ? (dark ? 'text-[#7FFF9B] font-bold' : 'text-[#235347] font-bold') : (dark ? 'text-[#e9e9e9]' : 'text-[#171717]')}`}>
                India
              </span>
            </div>
            <div className="flex flex-col items-center">
              <img
                ...
                alt="USA"
                className={`flex items-center justify-center ${selectedCountry === 'us' ? (dark ? 'outline outline-4 outline-[#7FFF9B] rounded-sm' : 'outline outline-4 outline-[#235347] rounded-sm') : ''} w-10 cursor-pointer`}
              />
              <span className={`text-xs pt-2 ${selectedCountry === 'us' ? (dark ? 'text-[#7FFF9B] font-bold ' : 'text-[#235347] font-bold') : (dark ? 'text-[#e9e9e9]' : 'text-[#171717]')}`}>
                USA
              </span>
            </div>
          </div>
