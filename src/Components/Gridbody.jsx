...
export default function Gridbody(props) {
 ...
 /*  const [articles, setArticles] = useState([]);
  ...
  const fetchNews = async () => {
    let url = `https://newsapi.org/v2/top-headlines?country=us&category=general&apiKey=e243412e09af41afa03e28dfa5ea65e8`;
    setLoading(true);
...
<div className='flex  justify-center items-center  max-lg:ml-[18rem] max-md:ml-[16rem] max-sm:ml-[3rem] ml-[20rem] mr-4 mt-24 overflow-y-auto'>
  ...
  {data.articles.map((item, index) => (
        <Card key={index} imageURL={item.urlToImage} title={item.title} newsUrl={item.url} description={item.description} source={item.source.name} timestamp={item.publishedAt} />
      ))}
  </div>
</div>
